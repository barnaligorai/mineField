...
echo '{"path":[3,13,23,33,34,35,36,46,56,57,67,77,87,86,96],"row":10,"col":10, "pos":-7, "choices":[]}' > ./src/mineField.json
# echo '{"path":[2,5,6,9,6],"row":3,"col":3, "pos":-1, "choices":[]}' > ./src/mineField.json
...
# Start game
node ./src/startGame.js 'u'
game_status=$(grep 'Over' ./src/mineField.json)

until [[ $? -eq 0 ]];do
  echo -e 'u : upward\tr : right\td : downward\tl : left\n'
  read -p 'Enter your move : ' move
  node ./src/startGame.js $move
  game_state=$(grep 'Over' ./src/mineField.json)
done
